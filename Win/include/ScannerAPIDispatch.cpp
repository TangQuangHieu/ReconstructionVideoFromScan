...
extern "C"
{
	IScannerDiscovery*	CreateScannerDiscoveryInstance()
	{
		IScannerDiscovery* scannerDiscovery = NULL;
		
		HRESULT result = CoCreateInstance(CLSID_CScannerDiscovery,
										  NULL,
										  CLSCTX_ALL,
										  IID_IScannerDiscovery,
										  (void**)&scannerDiscovery);
		if (FAILED(result))
		{
			std::cerr << "A scanner discovery instance could not be created: "
	...
	IScannerAPIInformation*	CreateScannerAPIInformationInstance()
	{
		IScannerAPIInformation* scannerAPIInformation = NULL;
		
		HRESULT result = CoCreateInstance(CLSID_CScannerAPIInformation,
										  NULL,
										  CLSCTX_ALL,
										  IID_IScannerAPIInformation,
										  (void**)&scannerAPIInformation);
		if (FAILED(result))
		{
			std::cerr << "A scanner API Information instance could not be created: "
					  << _com_error(result).ErrorMessage() << std::endl;
			return NULL;
		}

		return scannerAPIInformation;
	}
}
