...
{
public:
	TimeCode() : frameCountBCD(0x00000000) {}
	TimeCode(BMDScannerTimecodeBCD bcd) : frameCountBCD(bcd) {}
	TimeCode(unsigned fc, uint16_t filmFrameRate);
	TimeCode(const std::string& tc);
	~TimeCode() {};
	
	unsigned		toFrameCount(uint16_t filmFrameRate) const;	
	std::string		toString(uint16_t filmFrameRate) const;
	BMDScannerTimecodeBCD	toBCD() const;

private:
	BMDScannerTimecodeBCD	frameCountBCD;	
